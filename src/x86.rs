...
extern "sysv64" fn inthandler(info: &InterruptInfo, index: usize) {
    ...
    match index {
        ...
        14 => {
            ...
            error!(
                "Caused by: A{} mode {} on {} page, page structres are {}",
...
impl TaskStateSegment64 {
    ...
    pub fn new() -> Self {
        ...
        let this = TaskStateSegment64 {
            inner: Box::pin(tss64),
        };
        info!("TSS64 vreated @: {:#X}", this.phys_addr());
        this
    }
}
...
pub const BIT_DS_WRITEABLE: u64 = 1u64 << 41;
...
enum GdtAttr {
    ...
    KernelData = BIT_TYPE_DATA | BIT_PRESENT | BIT_DS_WRITEABLE,
}
...
impl GdtWrapper {
    pub fn load(&self) {
        ...
        info!("Loading TSS ( selector = {:#0X} )", TSS64_SEL);
        unsafe {
            asm!("ltr cx",
                in("cx") TSS64_SEL,
            );
...
impl fmt::Debug for GdtSegmentDescriptor {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
