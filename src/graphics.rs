...
unsafe fn draw_point<T: Bitmap>(buf: &mut T, color: u32, x: i64, y: i64) -> Result<()> {
...
fn draw_line<T: Bitmap>(buf: &mut T, color: u32, x0: i64, y0: i64, x1: i64, y1: i64) -> Result<()> {
    ...
    if dx >= dy {
        for (rx, ry) in (0..dx).flat_map(|rx| calc_slope_point(dx, dy, rx).map(|ry| (rx, ry))) {
            unsafe { draw_point(buf, color, x0 + rx * sx, y0 + ry * sy) }?;
        }
    } else {
    ...
    if let Some(font) = lookup_font(c) {
        for (dy, row) in font.iter().enumerate() {
            for (dx, pixel) in row.iter().enumerate() {
                let color = if *pixel == '*' { color } else { continue };
                let _ = unsafe { draw_point(buf, color, x + dx as i64, y + dy as i64) };
            }
        }
    }
}
...
pub fn draw_test_pattern(buf: &mut VramBufferInfo) {
    let w = 128;
